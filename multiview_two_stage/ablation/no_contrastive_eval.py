...
def _lock_no_contrastive_config():
    TRAIN_CONFIG['lambda_contrastive'] = 0.0
    TRAIN_CONFIG['score_weight_consistency'] = 0.0
...
def main(seed=42):
    set_random_seed(seed)
    print("=" * 80)
    print("Gate ON Evaluation (Two-stage training: Stage1 backbone + Stage2 gate) - no_contrastive")
    print("=" * 80)
    print(f"\nDataset: {DATA_NAME}")
    print(f"Config: lambda_recon={TRAIN_CONFIG['lambda_recon']}, "
          f"lambda_contrastive={TRAIN_CONFIG['lambda_contrastive']} (locked)")
    ...
    for combo_name, view_names in VIEW_COMBINATIONS.items():
        ...
        try:
            ...
            print(f"Loss weights: λ_recon={TRAIN_CONFIG['lambda_recon']}, λ_contrastive={TRAIN_CONFIG['lambda_contrastive']} (locked)")
            print(f"Score weights: recon={TRAIN_CONFIG.get('score_weight_recon', 0.3)}, "
                  f"consistency={TRAIN_CONFIG.get('score_weight_consistency', 0.0)} (locked)")
    ...
    if not results_df.empty:
        ...
        output_file = os.path.join(ablation_dir, f"{DATA_NAME}_no_contrastive_results.xlsx")
        ...
        if not results_df_auprc.empty:
            ...
            output_file_auprc = os.path.join(ablation_dir, f"auprc_{DATA_NAME}_no_contrastive_results.xlsx")
...
def main_multi(seeds):
    ...
    print("=" * 80)
    print("Gate ON Evaluation (Two-stage training, multiple random seeds) - no_contrastive")
    print("=" * 80)
    print(f"\nDataset: {DATA_NAME}")
    ...
    if not results_df.empty:
        ...
        output_file = os.path.join(ablation_dir, f"{DATA_NAME}_no_contrastive_results_seeds_{seeds_tag}.xlsx")
        ...
        if not results_df_auprc.empty:
            ...
            output_file_auprc = os.path.join(ablation_dir, f"auprc_{DATA_NAME}_no_contrastive_results_seeds_{seeds_tag}.xlsx")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Gate ON Evaluation (multiview_two_stage) - no_contrastive')
    ...
    parser.add_argument('--lambda_contrastive', type=float, default=1.0,
                        help='Contrastive learning loss weight (will be locked to 0)')
    ...
    parser.add_argument('--score_weight_recon', type=float, default=None,
                        help='Weight of reconstruction error in anomaly score (default: 0.3)')
    parser.add_argument('--score_weight_consistency', type=float, default=None,
                        help='Weight of consistency score in anomaly score (will be locked to 0)')
