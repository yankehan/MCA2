...
def _lock_no_ae_config():
    TRAIN_CONFIG['lambda_recon'] = 0.0
    TRAIN_CONFIG['score_weight_recon'] = 0.0
...
def main(seed=42):
    set_random_seed(seed)
    print("=" * 80)
    print("Gate ON Evaluation (Two-stage training: Stage1 backbone + Stage2 gate) - no_ae")
    print("=" * 80)
    print(f"\nDataset: {DATA_NAME}")
    print(f"Config: lambda_recon={TRAIN_CONFIG['lambda_recon']} (locked), "
    ...
    for combo_name, view_names in VIEW_COMBINATIONS.items():
        ...
        try:
            ...
            print(f"Loss weights: λ_recon={TRAIN_CONFIG['lambda_recon']} (locked), λ_contrastive={TRAIN_CONFIG['lambda_contrastive']}")
            print(f"Score weights: recon={TRAIN_CONFIG.get('score_weight_recon', 0.0)} (locked), "
    ...
    if not results_df.empty:
        ...
        output_file = os.path.join(ablation_dir, f"{DATA_NAME}_no_ae_results.xlsx")
        ...
        if not results_df_auprc.empty:
            ...
            output_file_auprc = os.path.join(ablation_dir, f"auprc_{DATA_NAME}_no_ae_results.xlsx")
...
def main_multi(seeds):
    ...
    print("=" * 80)
    print("Gate ON Evaluation (Two-stage training, multiple random seeds) - no_ae")
    print("=" * 80)
    print(f"\nDataset: {DATA_NAME}")
    ...
    for combo_name, view_names in VIEW_COMBINATIONS.items():
        ...
        try:
            ...
            for seed in seeds:
                ...
                print("\nResults:")
                print(f"  Average AUC: {auc_on:.4f} ± {auc_on:.4f} AUPRC: {auprc_on:.4f}")
                print(f"  Stage1 time: {stage1_time:.1f}s")
                print(f"  Stage2 time: {stage2_time:.1f}s")
    ...
    if not results_df.empty:
        ...
        output_file = os.path.join(ablation_dir, f"{DATA_NAME}_no_ae_results_seeds_{seeds_tag}.xlsx")
        ...
        if not results_df_auprc.empty:
            ...
            output_file_auprc = os.path.join(ablation_dir, f"auprc_{DATA_NAME}_no_ae_results_seeds_{seeds_tag}.xlsx")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Gate ON Evaluation (multiview_two_stage) - no_ae')
    ...
    parser.add_argument('--lambda_recon', type=float, default=1.0,
                        help='Reconstruction loss weight (will be locked to 0)')
    ...
    parser.add_argument('--score_weight_recon', type=float, default=None,
                        help='Weight of reconstruction error in anomaly score (will be locked to 0)')
